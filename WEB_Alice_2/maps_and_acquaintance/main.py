...
def play_game(res, req):
    ...
    if attempt == 1:
        ...
        if get_city(req) == city:
            ...
            res['response']['text'] = f'{sessionStorage[user_id]["first_name"]}, Правильно! А теперь угадай страну'
            res["response"]["buttons"] = [
                {
                    'title': 'Да',
                    'hide': True
            ...
            if attempt == 3:
                ...
                res['response']['text'] = f'{sessionStorage[user_id]["first_name"]}, Вы пытались'
                sessionStorage[user_id]['game_started'] = False
                sessionStorage[user_id]['guessed_cities'].append(city)
                return
        else:
            res["response"]["text"] = f'{sessionStorage[user_id]["first_name"]}, Я не знаю таких слов'
